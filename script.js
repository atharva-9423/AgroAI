document.addEventListener('DOMContentLoaded', () => {
    ...
    if (analyzeSoilCard && soilImageInput) {
        ...
        soilImageInput.addEventListener('change', (e) => {
            ...
            const validNames = ['alluvial-soil', 'black-soil', 'red-soil'];
            const fileNameLower = file.name.toLowerCase();
            const isValid = validNames.some(name => fileNameLower.includes(name));
            ...
            reader.onload = (ev) => {
                ...
                if (fileNameLower.includes('black-soil')) soilType = 'black-soil';
                if (fileNameLower.includes('red-soil')) soilType = 'red-soil';
                ...
                // Launch overlay
                launchSoilScan(ev.target.result, soilType);
            };
            reader.readAsDataURL(file);
    ...
    function launchSoilScan(imageSrc, soilType) {
        ...
        setTimeout(() => {
            ...
            sessionStorage.setItem('analyzedSoilType', soilType);
            window.location.href = 'soil_analysis.html';
        }, 600 + metrics.length * 500 + 1000); // extra 1 second to see the 100% complete state
